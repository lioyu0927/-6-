"""
TOUCHLESS WORKSTATION v12.FINAL_COMPLETE
─────────────────────────────────────────────────────────────
개선 사항 요약
 ...
 16. 오디오 스트림 예외 처리 및 [NEW] 샘플 레이트 하드웨어 동적 가변 오토 매칭 적용
 17. 모바일 게임 톤앤매너 전면 UI 개편 — 상단 타이틀 바, 우측 3단 메뉴 버튼 인터페이스 적용
 18. [NEW] 우측 메뉴바 영역 마우스 클릭(Click) 상호작용 지원
 19. [NEW] 좌우 조작 영역 점선 가이드 및 PINCH 조작 시 네온 타겟 링 피드백 시각화
"""
...
class TouchlessWorkstation:
    ...
    def process_gestures(self, results, frame_shape, canvas):
        ...
        # [NEW] 좌우 배치 전용 십자 점선 조작 그리드 선 시각화
        cv2.line(canvas, (capture_w // 2, 55), (capture_w // 2, h), tuple(max(c//2, 15) for c in color), 1, cv2.LINE_AA)
        ...
        for idx, (hand_lms, gesture) in enumerate(zip(lms_list, gestures)):
            ...
            for pt_idx in [0, 4, 8, 12, 16, 20]:
                pt = lms[pt_idx]
                cv2.drawMarker(canvas, (int(pt.x * w), int(pt.y * h)), (180, 180, 180), cv2.MARKER_CROSS, 10, 1)

            lbl_y = max(int(lms[0].y * h) - 22, 18)
            cv2.putText(canvas, gesture, (int(lms[0].x * w) - 24, lbl_y), cv2.FONT_HERSHEY_PLAIN, 1.1, color, 1)

            norm_x = float(np.clip(itip.x * (w / capture_w), 0.0, 1.0))
            iy_norm = float(np.clip(1.0 - itip.y, 0.0, 1.0))
            ...
            if norm_x > 0.5: 
                if gesture == "PINCH":
                    ...
                    # [NEW] PINCH 타겟 스틱 링 연출 효과
                    cv2.circle(canvas, (ix, iy), 18, (255, 255, 255), 2, cv2.LINE_AA)
                    cv2.circle(canvas, (ix, iy), 6, color, -1)
                elif gesture == "FIST":
                    self._set_target("Distortion", iy_norm)
                    self._set_target("Glitch", (norm_x - 0.5) * 2)
            else: 
                if gesture == "PINCH":
